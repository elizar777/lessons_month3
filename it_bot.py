...
async def get_id(message:types.Message):
    await message.answer(f"name: {message.from_user.full_name}, id: {message.from_user.id}")
...
async def start_mailing(message:types.Message):
    if message.from_user.id in [1488715778,]:
...
async def start_lids(message: types.Message):
    ...
    await message.answer(f"Как: Имя, Фамилия, Направление, Номер")
...
async def get_direction(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"Введите направления: ")
...
async def get_phone(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"Введите номер телефонв: ")
...
async def save_lids(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"Saving informations...")
